...
sayHelloButton.addEventListener('mouseover', buttonColor);
// PROBLEM 3
/*
...
sayHelloButton.addEventListener('mouseout', colorChangeBack);
// PROBLEM 4
/*
...
const repeatMyParam = () => {
 ...
 axios.get(`${localUrl}/repeat/'someparam'`) 
 .then((response) => {
    ...
    repeatText.textContent = response.data
     for(let i = 0; i < response.data.length; i++) {
         let makePara = document.createElement('p')
           document.body.appendChild(makePara)
         }
    //    document.appendChild(document.createElement('p').classList.add(`ohAnimals${[i]}`))
    //    document.querySelector(`ohAnimals${[i]}`).textContent = response.data[i]
     })
...
// CODE HERE
let queryRequest = () => {
    axios.get(`${localUrl}/query-test/?name = tiger&?name = dog`)
    .then((response) =>{
        console.log(response.data)
    })
}
